...
def _render_summary_text(server_results, sort_key):                       #---------- PROBADO ----------#
    ...
    for r in sorted(server_results, key=lambda r: sort_key(r["serv_name"])):
        if r["problem"] is not None:
            # El detalle del error ya está en el log de esa planta; aquí solo
            # interesa saber que no se revisó ninguna cámara — "-/-" en vez de
            # "0/0" para no insinuar que sí se determinó un total (el
            # problema fue previo a saber cuántas cámaras hay). Mismo
            # criterio para [SIN YAML] y [SIN CONEXIÓN]: el estado ya dice
            # el motivo general, y para el detalle exacto se va al log.
