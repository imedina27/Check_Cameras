...
- [ ] **Definir y crear los procesos de paralelización.** Paralelizar las cámaras **dentro de un mismo servidor** con `concurrent.futures.ThreadPoolExecutor` (I/O-bound, no requiere `asyncio`). Servidores entre sí se mantienen secuenciales — ver nota abajo.
...
- [x] **Logging seguro para concurrencia.** Resuelto de forma anticipada en Fase 3: al migrar `log_processor()` al módulo `logging`, los `Handler` (`FileHandler`/`StreamHandler`) ya traen su propio candado interno — no hace falta agregar un `threading.Lock()` manual.
