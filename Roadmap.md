...
- [ ] **Revisión y homologación de archivos de configuración — investigación por marca (en progreso).** `dic_to_json()` (Axis/Dahua) y `txt_to_json()` (Vivotek) parsean el texto plano de configuración de cada marca con reglas distintas entre sí (separador `.` vs `_`, comillas en el valor, prefijo a omitir) porque así responde cada cámara — no es necesariamente algo a "unificar" a la fuerza. Sigue pendiente evaluar si conviene una función común parametrizada en vez de varias casi-iguales, y revisar Hikvision/Dahua/Vivotek con el mismo método que Axis (ver abajo).
  ...
  - **[ ] Hikvision, Dahua, Vivotek — pendientes** de la misma revisión (qué tan completa es la configuración que se pide, si hay un endpoint de "todo en una sola petición", y si el parser pierde algo).
